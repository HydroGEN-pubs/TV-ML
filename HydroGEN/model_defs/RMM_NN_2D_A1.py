...
class RMM_NN(nn.Module):
    def __init__(self, grid_size=[25, 25],  channels=2, verbose=False,):
        ...
        c2d_s3 = shp.conv2d_shape(input_shape=c2d_s2, cout=Cout, padding=cnv_padding,
                              kernel_size=cnv_kernel_size, dilation=cnv_dilation, stride=cnv_stride)
        ...
        # Linear Steps
        # ---------------------------------------------------------------------
        L_Fin1 = int(np.prod(np.floor(c2d_s3)))
        ...
        #L_Fout1 = 10000  # dense layer size from NN w/ BC, very sensitive parameter, set to 100, 500, 1000
