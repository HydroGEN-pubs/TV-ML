...
class RMM_NN(nn.Module):
    ...
    def forward(self, x):
        out = self.contract(x)
        if self.verbose: print('contract ', out.shape)
        #conv2 = self.contract(conv1)
        #conv3 = self.contract(conv2)

        #upconv3 = self.expand(conv3)
        #upconv2 = self.expand(upconv3)
        out = self.expandfin(out)
        if self.verbose: print('expand ', out.shape)
