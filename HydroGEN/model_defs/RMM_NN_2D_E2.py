...
class RMM_NN(nn.Module):
    def __init__(self, grid_size=[25, 25],  channels=2, verbose=False):
        ...
        c2d_s3 = shp.conv2d_shape(input_shape=pool2d_s1, cout=Cout, padding=cnv_padding,
                              kernel_size=cnv_kernel_size, dilation=cnv_dilation, stride=cnv_stride)
        c2d_s4 = shp.conv2d_shape(input_shape=c2d_s3, cout=Cout, padding=cnv_padding,
                              kernel_size=cnv_kernel_size, dilation=cnv_dilation, stride=cnv_stride)
        pool2d_s2 = shp.pool2d_shape(input_shape=c2d_s4, padding=pool_padding, kernel_size=pool_kernel_size,
                                 dilation=pool_dilation, stride=pool_stride)
        trans2d_s1 = shp.convTranspose2d_shape(input_shape=pool2d_s2, cout=Cout, padding=cnv_padding,
                            kernel_size=cnv_kernel_size, dilation=cnv_dilation, stride=cnv_stride)
    ...
    def forward(self, x):
        ...
        #print('contract1 ', conv1.shape)
        conv2 = self.contract2(conv1)
        #conv3 = self.contract(conv2)
        #print('contract2 ', conv2.shape)

        #upconv3 = self.expand(conv3)
        upconv2 = self.expand1(conv2)
        #print('expand1 ', upconv2.shape)
        out = self.expand2(torch.cat([upconv2, conv1], 1))   #upconv2
        #print('expand2 ', out.shape)
