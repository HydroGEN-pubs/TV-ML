...
# %%
# USER settings
#Run selecion settings
get_from_csv = False #if false run list will be generated from directory
...
batch_size = 1024 #batch size for training set =-1 to make the batch size equal to #runs in the ensemble
N = -1  #time steps per run to use set =-1 to set this by the run length.

## NOTE THAT: total batch size will be batch_size * N
...
print(pf_path,TV_config,train_ens)
...
# Create a a directory for the run if it doesn't already exist
...
print(os.path.join(run_path, (this_file[:-3]+"_" +run_config+'.py')))
...
print("Slicing on axis", slice_axis, "Selecting Layer", lpick)
...
model = mldef.RMM_NN(grid_size=[n_dim2, n_dim1],
               channels=nchannel)
model.to(DEVICE)
...
if LBFGS != True:
    ...
    optimizer = torch.optim.LBFGS(model.parameters(), lr=learning_rate, max_iter=20, tolerance_grad=1e-7, tolerance_change=1e-9, history_size=20, line_search_fn='strong_wolfe')

# Setup transforms to get variables scaled from 0-1
PRESS_TRANSFL10 = float32_clamp_scaling(src_range=[0.0, 0.063], dst_range=[0, 1])
...
# because we are predicting from one time to the next, the timesteps become part of the batch

input_temp = np.zeros((n_run*N, nchannel, n_dim2, n_dim1))
output_temp_temp = np.zeros((D_out))
...
for rr in range(n_run):
    ...
    if print_verbose == True:
        print()
        print("-----------------------------------------------")
        print(f'{progress}% Training on {run_definition}')
        print()
        print("-- Precip Inputs --")
        print("Max Precip [m/h]:", maxp)
        print("Time of Application [steps]:", (time_application))
        # ___
    ...
    #count = 0
    for ii in range(0, N):
        # make continuous batch vector
        iii = (rr)*N + ii
        print(f'batch {iii} ', end='\r')
        # Static Variable are slopes
        input_temp[iii, 0, :, :] = SLOPE_TRANSF(PFslice2D(data.slope_x, slice_axis, lpick))
        input_temp[iii, 1, :, :] = SLOPE_TRANSF(PFslice2D(data.slope_y, slice_axis, lpick))
        input_temp[iii, 2, :, :] = MANN_TRANSF(PFslice2D(data.mannings, slice_axis, lpick))
        # precip is time dependent
        if (ii<=time_application):
            input_temp[iii, 3, :, :] = MAXP_TRANF(Max_Precip)
            
        else:
            input_temp[iii, 3, :, :] = MAXP_TRANF(Zero_Precip)
        data.time = ii

        input_temp[iii, 4,:, :] = PRESS_TRANSFL10(
            PFslice2D(data.pressure, slice_axis, lpick))

        data.time = ii+1

        output_temp[iii,:] = np.reshape(PRESS_TRANSFL10(
            PFslice2D(data.pressure, slice_axis, lpick)), n_grid)
...
for epoch in range(n_epoch):
    ...
    for bb in range(int(n_batch)):
        ...
        b_from = (bb*batch_size*N)
        b_to = (batch_size_temp*N) + (bb*batch_size*N)
        ...
        if LBFGS == True:
            for epoch in range(n_epoch):    
                ...
                print("Epoch: %3d, batch: %3d,loss: %5.3e, memory (Mb): %6.2f" % (epoch, bb, loss, memory), end='\r')
